...
export default function App() {
  ...
  return (
    <div className="container">
      ...
      <MDEditor value={value} preview="edit" extraCommands={[codePreview, customButton, commands.fullscreen]} onChange={(val) => setValue(val)} />
    </div>
  );
}
```
...
export default function App() {
  const [value, setValue] = React.useState('**Hello world!!!**');
  return <MDEditor value={value} preview="edit" commands={[...commands.getCommands(), help]} onChange={(val) => setValue(val)} />;
}
```
...
export default function App() {
  const [value, setValue] = React.useState('**Hello world!!!**');
  return (
    <MDEditor value={value} preview="edit" commands={[...getCommands()]} extraCommands={[...getExtraCommands()]} onChange={(val) => setValue(val)} />
  );
}
```
