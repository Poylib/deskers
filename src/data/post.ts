...
import { CategoryDetail, HeadingItem, Post, PostMatter } from '../_config/types';
...
export const parsePostAbstract = (postPath: string) => {
  ...
  const url = `/tail/${categoryPath}/${slug}`;
...
export const getSortedPostList = async (category?: string) => {
  const postList = await getPosts(category);
...
export const getAllPostCount = async () => (await getPosts()).length;
